      ...
      <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        {documents.map((doc) => (
          <li key={doc.id} className="flex">
            <Link
              to={`/documents/${doc.id}`}
              className="flex w-full flex-col rounded-2xl border border-line bg-surface p-5 transition-all duration-150 ease-serene hover:border-orange-300 hover:shadow-sm">
              
              <span className="label-eyebrow text-ink-muted">{doc.reference}</span>
              <span className="mt-1.5 font-serif text-lg font-semibold leading-snug text-ink">{doc.title}</span>
              ...
              <span className="mt-auto flex items-center gap-2 pt-4 text-xs text-ink-muted">
                ...
                <span className="font-medium text-ink-soft">{doc.role}</span> · {doc.branches.length} branch{doc.branches.length > 1 ? 'es' : ''}
              </span>
            </Link>
          </li>
        ))}
      ...
      {documents.map((doc) => (
        <li key={doc.id}>
          <Link
            ...
            <span
              aria-hidden="true"
              className="flex h-9 w-9 items-center justify-center rounded-lg bg-orange-100/70 text-orange-700 border border-orange-200">
